...
def not_modifiable_path_files_init():
    ...
    global tout
    global syrup_flags
    syrup_flags = syrup_encoding_flags + " -tout " + str(tout) + " -solver oms"
...
def analyze_file(solution):
    global solver
    if solver == "oms":
        analyze_file_oms(solution)
    elif solver == "z3":
        analyze_file_z3(solution)
    else:
        analyze_file_barcelogic(solution)
    ...
    for contract_path in [f.path for f in os.scandir(contracts_dir_path) if f.is_dir()]:
        ...
        for file in glob.glob(contract_path + "/*.json"):
            ...
            if tout_pattern:
                ...
                target_gas_cost, shown_optimal = analyze_file_oms(solution)
