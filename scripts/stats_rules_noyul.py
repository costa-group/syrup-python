...
def main():
    ...
    content = ["Contract,normal-constant,normal-r1,normal-r2,opt-constant,opt-r1,opt-r2,nooyul-constant,noyul-r1,noyul-r2,lines-opt,lines-normal,lines-noyul,gas-normal,gas-opt,gas-noyul,higher-constant,higher-r1,higher-r2,higher,max-lines,max-gas"]#nooyul-constant,noyul-r1,noyul-r2


    content_gas = ["Contract,normal-lines,opt-lines,no-yul-opt,normal-syrup-lines,opt-syrup-lines,noyul-syrup-lines,normal-gas,opt-gas,noyul-gas,normal-syrup-gas,opt-syrup-gas,noyul-syrup-gas"]
    ...
    for f in files:
        ...
        ls_noyul, gs_noyul, bl_noyul = compute_info_noyul(sname)
        ...
        y1 = count_constant_expressions(lines_noyul)
        ...
        mt = max (n1+n2+n3,o1+o2+o3,y1+y2+y3) #y1+y2+y3
        if mt == n1+n2+n3:
            mt = "NORMAL"
        ...
        new_line = ",".join([f,str(n1),str(n2),str(n3),str(o1),str(o2),str(o3),str(y1),str(y2),str(y3),str(l1),str(l2),str(l3),str(g1),str(g2),str(g3),m1,m2,m3,mt,max_l,max_g])#y1,y2,y3
        ...
        p3 = (g3-gs_noyul)*100.0/g3
        
        new_line2 = ",".join([f,str(l1),str(l2),str(l3),str(ls_normal),str(ls_opt),str(ls_noyul),str(g1),str(g2),str(g3),str(gs_normal),str(gs_opt),str(gs_noyul),str(p1),str(p2),str(p3)])
        new_line2+="\n"+"****************************************************\n"
        new_line2+=str(b1)+","+str(bl_normal)+","+str(b2)+","+str(bl_opt)+","+str(b3)+","+str(bl_noyul)
