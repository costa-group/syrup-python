...
def clean_dir():
    ...
    if syrup_folder in os.listdir(tmp_path):
        for elem in os.listdir(syrup_path):
            last = elem.split(".")[-1]
            if last in ext:
                os.remove(syrup_path+elem)

        if "jsons" in os.listdir(syrup_path):
            shutil.rmtree(syrup_path + "jsons")

        if "disasms" in os.listdir(syrup_path):
            shutil.rmtree(syrup_path + "disasms")

        if "smt_encoding" in os.listdir(syrup_path):
            shutil.rmtree(syrup_path + "smt_encoding")

        if "solutions" in os.listdir(syrup_path):
            shutil.rmtree(syrup_path + "solutions")
...
def generate_saco_hashes_file(dicc):
    with open(syrup_path+"solidity_functions.txt", "w") as f:
        for name in dicc:
...
def main():
    ...
    if args.isolate_block:
        exit_code = analyze_isolate_block()
    ...
    six.print_("The files generated by EthIR are stored in the following directory: "+syrup_path)
